...
int knot_thing_create_schema(uint8_t i, knot_msg_schema *schema)
{
	knot_msg_schema msg;
	...
	if (data_items[i].name != KNOT_THING_EMPTY_ITEM) {
		...
		entry->values.name = data_items[i].name;

		msg.hdr.payload_len = sizeof(entry->values) +
					sizeof(entry->sensor_id);

		memcpy(&msg.values, &entry->values,
						sizeof(entry->values));
		/*
...
static int data_item_read(uint8_t sensor_id, knot_data *data)
{
	...
	if (data_items[sensor_id].name != KNOT_THING_EMPTY_ITEM) {
		...
		case KNOT_VALUE_TYPE_FLOAT:
			...
			data->values.val_f.value_dev = uint32_val;
