...
static int get_data(knot_msg_data *data)
{
	...
	/*
	 * FIXME: Payload len is always raw size, need to check the data type
	 * and get the real size
	 */
	msg.hdr.payload_len = sizeof(msg.payload);
...
int knot_thing_protocol_run(void)
{
	...
	case STATE_ONLINE:
		 ...
		 * verify_events(i, data) and use comm_write to
